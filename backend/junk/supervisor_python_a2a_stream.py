...
async def build_tools_from_registry(
    ...
    def _create_tool_for_card(card: AgentCard) -> StructuredTool:
        ...
        async def tool_impl(content: str, context: Dict[str, Any] = {}):
            ...
            agent_client = StreamingClient(endpoint_url=card.url)

            try:
                # Use the ask method for simple text queries
                logger.info(f"Sending message to {card.url}")
                response = agent_client.stream_response(Message(content=TextContent(text=content), role=MessageRole.AGENT, metadata={**context}))
                async for chunk in response:
                    logger.info(f"Received response from {card.url}")
                    logger.debug(
                        f"Tool {card.name} returned response: {str(chunk)[:200]}..."
                    )
                    yield chunk
                ...
                yield f"Error communicating with agent {card.name}: {str(e)}"
...
class Supervisor(A2AServer):
    ...
    def handle_task(self, task: Task):
        ...
        try:
            ...
            result = asyncio.run(self.process_orchestration(text, {
                "task_id": task.id,
    ...
    async def orchestrate(
        ...
        result = self.graph.astream(inputs, config=config, stream_mode='values')
        async for item in result:
            logger.debug(f"Graph invocation completed, result keys: {list(item.keys())}")
        ...
        yield {"ok": True, "content": final, "raw": result}
    
    async def process_orchestration(self, content: str, context: Optional[Dict[str, Any]] = None, task=None):
        """Process orchestration results in real-time as they arrive and return the final result."""
        logger.info(f"Processing orchestration with LangGraph: {content[:100]}...")
        logger.debug(f"Task context: {context}")
        ...
        final_result = None
        async for result in self.graph.astream(inputs, config=config, stream_mode='values'):
            # Process each chunk immediately as it arrives
            await self.process_chunk(result, task)
            final_result = result
        
        logger.debug(f"Graph invocation completed, final result keys: {list(final_result.keys()) if final_result else 'None'}")

        if final_result:
            msgs = final_result.get('messages', [])
            final = msgs[-1].content if msgs else ""
            logger.info(f"Task completed, final response length: {len(final)}")
            return {"ok": True, "content": final, "raw": final_result}
        else:
            return {"ok": False, "content": "No response generated", "raw": None}

    async def process_chunk(self, chunk, task):
        """Process a single chunk of the orchestration result and update task in real-time."""
        logger.debug(f"Processing chunk: {chunk}")
        
        # Extract useful information from the chunk
        ...
        if messages:
            ...
            if hasattr(latest_message, 'tool_calls') and latest_message.tool_calls:
                # Tool is being called - update task status
                tool_name = latest_message.tool_calls[0].get('name', 'Unknown tool')
                task.status = TaskStatus(
                    state=TaskState.WAITING,
                    message={"role": "agent", "content": {"type": "text", "text": f"Executing tool: {tool_name}..."}}
                )
                logger.info(f"Task {task.id}: Executing tool {tool_name}")
            
            # Check if this is a tool response
            elif hasattr(latest_message, 'content') and latest_message.content:
                # Tool response received - update task with intermediate result
                content = str(latest_message.content)
                if len(content) > 100:
                    content = content[:100] + "..."
                
                # Add intermediate artifact for this step
                intermediate_artifact = {
                    "parts": [{"type": "text", "text": f"Step completed: {content}"}],
                    "step": len(messages),
                    "timestamp": time.time()
                }
                
                if not hasattr(task, 'artifacts'):
                    task.artifacts = []
                task.artifacts.append(intermediate_artifact)
                
                # Update task status to show progress
                task.status = TaskStatus(
                    state=TaskState.WAITING,
                    message={"role": "agent", "content": {"type": "text", "text": f"Processing step {len(messages)}..."}}
                )
                
                logger.info(f"Task {task.id}: Step {len(messages)} completed")
        
        # Could also:
        # - Send real-time updates to connected clients
        # - Update a progress bar or status indicator
        # - Store intermediate results in a database
